...
greatest.dep.delay <- flights %>% group_by(month) %>% summarise(delay = mean(dep_delay, na.rm = TRUE ))
...
# Hint: you'll have to perform a grouping operation before summarizing your data
highest.arr.delay <- flights %>% group_by(dest) %>% summarise(delay = max(mean(arr_delay, na.rm = TRUE)))
