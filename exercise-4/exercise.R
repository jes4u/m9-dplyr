...
flights <- mutate(flights, gained.in.air = arr_delay - dep_delay)

# Sort your data.frame desceding by the column you just created
flights <- arrange(flights, -gained.in.air)

# Try doing the last 2 steps in a single operation using the pipe operator
flights <- flights %>% mutate(flights, gained.in.air = arr_delay - dep_delay) %>% arrange(desc(gained.in.air))

# Make a histogram of the amount of gain using the `hist` command
hist.flights <- hist(flights$gained.in.air)

# On average, did flights gain or lose time?
mean(flights$gained.in.air, na.rm = TRUE)

# Create a data.frame that is of flights headed to seatac ('SEA'), 
flights.to.SEA <- flights %>% select(gained.in.air, dest) %>% filter(dest == "SEA")

# On average, did flights to seatac gain or loose time?
mean(flights.to.SEA$gained.in.air, na.rm = true)
